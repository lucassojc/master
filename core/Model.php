<?php
    ...
    abstract class Model {
        ...
        public function getBySessionNumber(string $sessionNumber, $value) {
            if(!$this->isFieldValueValid($sessionNumber, $value)) {
                throw new \Exception('Invalid field name or value' . $sessionNumber);
            }

            $tableName = $this->getTableName();
            $sql = 'SELECT cart_id FROM ' . $tableName . ' WHERE ' . $sessionNumber . ' = ?;';
            $prep = $this->dbc->getConnection()->prepare($sql);
        ...
        final private function checkFieldList(array $data) {
            ...
            foreach ( $requestedFieldNames as $requestedFieldName) {
                if ( !in_array($requestedFieldName, $supportedFieldNames)) {
                    throw new \Exception('Field ' . $requestedFieldName . ' in not supported!');
                }

                if ( !$fields[$requestedFieldName]->isEditable() ) {
                    throw new \Exception('Field ' . $requestedFieldName . ' in not aditable!');
                }

                if ( !$fields[$requestedFieldName]->isValid($data[$requestedFieldName]) ) {
                    throw new \Exception('The value for the field ' . $requestedFieldName . ' in not valid!');
                }
            }
        }
